...
- [ ] **Chạy SQL `0017_reminder_tick.sql`** (cron phụ 'reminder-tick' MỖI PHÚT, SQL-gated) — để "nhắc tôi 5 phút nữa" bắn đúng phút thay vì trễ tối đa 15' theo cron chính. NHỚ thay `<PROJECT_REF>` + `<SERVICE_ROLE_KEY>` (legacy service_role JWT dạng eyJ..., giống lần sửa cron 0004) trước khi chạy.
