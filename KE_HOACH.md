...
## Nhật ký thay đổi
- 2026-06-20: Thương hiệu "Nofy". Đổi tên app hiển thị `ai-notifier-new` → **"Nofy"** (app.json name; slug/package giữ nguyên). Icon mới: cắt từ logo Nofy gốc (`assets/images/nofy-soure.png`) bằng Pillow (scripts/crop_icon.py), chọn tông COOL tím→cyan (scripts/finalize_icon.py) → `icon.png` + `adaptive-icon.png` (backgroundColor #FEFEFE, bỏ background/monochrome cũ). CHƯA BUILD — gộp với các thay đổi sau rồi build 1 thể. Scripts ở scripts/ (make_icon=vẽ tay đã bỏ, crop_icon, variants, finalize_icon).
