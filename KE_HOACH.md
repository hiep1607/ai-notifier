...
## Nhật ký thay đổi
- 2026-06-20: Chuẩn bị build Web + Android. Web: `npx expo export --platform web` → ra `dist/` OK. Android: thêm `android.package`/`ios.bundleIdentifier` = `com.hiep1607.ainotifiernew` vào app.json, tạo `eas.json` (profile `preview` = APK cài thẳng). CÒN CHỜ USER: `eas-cli login` + `eas init` (ghi projectId, cần cho push token) + `eas build -p android --profile preview`. Push Android cần thêm FCM credentials (Firebase) — làm sau khi có build.
