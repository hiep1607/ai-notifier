...
+ yêu cầu, chỉ đổi đúng phần được yêu cầu, giữ nguyên phần còn lại. App so sánh từng trường, hiện
**tóm tắt thay đổi** (vd "• Lịch: Hằng ngày 08:00 → Hằng ngày 07:00") để xác nhận rồi mới lưu.
Yêu cầu mơ hồ → AI hỏi lại; yêu cầu vi phạm (định kỳ <30 phút, đòi đổi sang Facebook...) → giải thích
như luồng tạo rule. Không đổi được LOẠI rule qua chat (url → tin tức...) — tạo rule mới cho lành.
...
- **Sau khi nhắc xong (nâng cấp 2026-07-03, cần migration 0021):** rule **TỰ XÓA HẲN** — không còn
  nằm ở tab Rules nữa, nhưng **thông báo nhắc VẪN CÒN** trong app (bằng chứng "app đã nhắc gì, lúc
  nào"). Kỹ thuật: notifications có cột chủ sở hữu trực tiếp `user_id` (0021); server gán user_id +
  gỡ `rule_id` cho thông báo TRƯỚC khi xóa rule → FK cascade không kéo thông báo theo; RLS + mọi màn
  client lọc theo user_id (lib/notifQuery, có fallback rule_id khi 0021 chưa chạy → khi đó giữ hành
  vi cũ: tắt rule + gom vào mục "Nhắc hẹn đã xong" ở tab Rules).
...
- **Trang cần ĐĂNG NHẬP — flow "cấp quyền" (làm lại 2026-07-03 cho dễ dùng):** app TỰ phát hiện trang
  ...
  `watch_auth` — RLS chặn user khác đọc, chỉ chủ rule + hệ thống quét (service_role) thấy.
  **Đăng nhập 1-CHẠM trên mobile (thêm 2026-07-03, màn `grant-login`):** bấm "Đăng nhập trong app" →
  trang mở TRONG APP (WebView), đăng nhập như bình thường, app tự gom cookie của trang → bấm nút
  **"Cấp quyền"** là xong, khỏi copy tay. Chạy được ngay trong Expo Go (webview có sẵn). *Giới hạn:
  trang giấu phiên kiểu **HttpOnly** thì JS không đọc được cookie → quay về cách dán thủ công (app có
  nhắc); trên WEB trình duyệt chặn đọc phiên trang khác → luôn dùng cách thủ công.*
...
- **Đúng giờ ±1 phút (tick, migration 0022):** cron tick chạy MỖI PHÚT (mở rộng từ reminder-tick 0017)
  xử lý nhắc hẹn + rule ghim giờ vừa tới mốc — bản tin 8:00 nổ đúng 8:00, không còn lệch 0-15' theo nhịp
  cron chính. Tick chỉ nhận rule trong cửa sổ [giờ hẹn, +10'); có **claim nguyên tử** trên `last_run_at`
  nên tick + cron chính chạy chồng cũng không bắn đúp.
- **Catch-up 4 tiếng:** nếu lượt đúng giờ hẹn bị lỡ (quota Gemini cạn, quá tải, deadline xử lý),
  cron chính 15' tự thử lại trong vòng 4 tiếng kể từ giờ hẹn — không còn cảnh "lỡ 1 lượt là
  mất nguyên ngày". (Tick lỗi quota cũng tự trả lại `last_run_at` cũ để catch-up còn tác dụng.)
- Ví dụ: rule "thời tiết Hà Nội hằng ngày 08:00" → 08:00:xx nhận push; nếu 8h hệ thống kẹt quota thì
  8:15/8:30... cron chính bắn bù.
...
| **Dedup 2 lớp: tiêu đề + LINK** | Đường RSS/feed lọc thêm theo `source_url` chuẩn hóa (bỏ `utm_*`/`fbclid`/fragment, bỏ www + trailing slash) — báo sửa tít nhẹ nhưng cùng link vẫn bị chặn. Ví dụ: "Vàng lên 76 triệu" sửa thành "Vàng lên 76 triệu, cao nhất tháng" cùng URL → không báo lại. |
| **Hash-gate trang URL** (`last_content_hash`, migration 0023) | Rule theo dõi URL: sau khi fetch, so VÂN TAY nội dung trang với lần trước — trang Y NGUYÊN → skip ngay, **0 call AI** (trước đây mỗi lượt đốt 1 call flash-lite dù trang không đổi). Không áp cho quét manual ("Kiểm tra tin ngay") và rule ghim giờ (bản tin đúng hẹn luôn giao). |
| **Cache RSS trong 1 lượt quét** | Nhiều rule cùng category (vd 5 rule "news") trong 1 lượt cron chỉ fetch bộ feed báo VN **1 lần** — đỡ chậm + đỡ tải feed. |
...
- **Loading nhanh (2026-07-03):** Home / tab Thông báo / tab Rules dùng cache "hiện ngay, làm mới
  ngầm" (`lib/screenCache`) — mở app hay quay lại tab là thấy dữ liệu lần trước NGAY, bản mới thay
  vào khi mạng trả về; truy vấn chạy song song; danh sách thông báo chỉ tải 100 dòng gần nhất
  (tìm kiếm/lọc áp trên 100 dòng đó).
- **Sức khỏe rule (2026-07-03, cần migration 0020):** chi tiết rule hiện dòng **"Quét lần cuối: X phút
  trước"** + hộp cảnh báo ⚠️ khi lần quét gần nhất lỗi (cột `rules.last_error` — run-monitor ghi lỗi
  mỗi lượt, quét êm thì xóa; lỗi đọc trang trước đây bị nuốt im giờ cũng hiện ra). Trả lời ngay câu
  "sao rule này im?" mà không cần vào trang admin.
- **Watchdog cron (2026-07-03, trong migration 0020):** job pg_cron THUẦN SQL mỗi 15 phút soi bảng
  `cron_runs` — quét nền ngừng chạy quá 50 phút (vd key cron sai → 401 như sự cố 2026-07-03) là đẩy
  push thẳng cho admin qua Expo Push API (không đi qua Edge Function nên không chết chùm). Chống spam:
  tối đa 1 cảnh báo/6h (bảng `watchdog_alerts`).
